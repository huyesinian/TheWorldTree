...
{
    public class TreeGALikeEX
    {
        public RubbishSel Rubbish;
        public TheWorldTreeDBContext _context;

        public TreeGALikeEX(TheWorldTreeDBContext context)
        {
        ...
        {
            var SelResult = Rubbish.GetList<TreeGALike>();//获取集合
            var Sum = SelResult.Count();//获取总数量
            if (searchInfo != null)
            {
                SelResult = SelResult.Where(x => x.LikeMan.Contains(searchInfo) || x.UseModule.Contains(searchInfo)).ToList();
            }
