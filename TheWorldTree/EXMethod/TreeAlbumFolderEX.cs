...
namespace TheWorldTree.EXMethod
{
    ...
    /// 相册文件夹扩展类
    /// </summary>
    public class TreeAlbumFolderEX
    {
        public RubbishSel Rubbish;
        public TheWorldTreeDBContext _context;

        public TreeAlbumFolderEX(TheWorldTreeDBContext context)
        {
        ...
        {
            var SelResult = Rubbish.GetList<TreeAlbumFolder>();//获取集合
            var Sum = SelResult.Count();//获取总数量
            if (searchInfo != null)
            {
                SelResult = SelResult.Where(x => x.FlName.Contains(searchInfo) || x.Describe.Contains(searchInfo)).ToList();
            }
