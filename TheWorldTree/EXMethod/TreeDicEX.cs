...
namespace TheWorldTree.EXMethod
{
    ...
    /// 字典扩展
    /// </summary>
    public class TreeDicEX
    {
        public RubbishSel Rubbish;
        public TheWorldTreeDBContext _context;

        public TreeDicEX(TheWorldTreeDBContext context)
        {
        ...
        {
            var SelResult = Rubbish.GetList<TreeDic>();//获取集合
            var Sum = SelResult.Count();//获取总数量
            if (searchInfo != null)
            {
                SelResult = SelResult.Where(x => x.DicCode.Contains(searchInfo) || x.NodeName.Contains(searchInfo) || x.NodeCode.Contains(searchInfo)).ToList();
            }
