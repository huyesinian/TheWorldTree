...
namespace TheWorldTree.EXMethod
{
    ...
    /// 新闻扩展类
    /// </summary>
    public class TreePressEX
    {

        public RubbishSel Rubbish;
        public TheWorldTreeDBContext _context;

        public TreePressEX(TheWorldTreeDBContext context)
        {
        ...
        {
            var SelResult = Rubbish.GetList<TreePress>();//获取集合
            var Sum = SelResult.Count();//获取总数量
            if (searchInfo!=null)
            {
                SelResult = SelResult.Where(x => x.Title.Contains(searchInfo) || x.MainBody.Contains(searchInfo) || x.Description.Contains(searchInfo)).ToList();
            }
