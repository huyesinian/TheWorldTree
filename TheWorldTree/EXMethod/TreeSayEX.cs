...
namespace TheWorldTree.EXMethod
{
    ...
    /// 说说扩展类
    /// </summary>
    public class TreeSayEX
    {
        public RubbishSel Rubbish;
        public TheWorldTreeDBContext _context;

        public TreeSayEX(TheWorldTreeDBContext context)
        {
        ...
        {
            var SelResult = Rubbish.GetList<TreeSay>();//获取集合
            var Sum = SelResult.Count();//获取总数量
            if (searchInfo != null)
            {
                SelResult = SelResult.Where(x => x.SayContent.Contains(searchInfo)).ToList();
            }
